...
con = connect(host='localhost',database='bhuvana',user='root',password='')
cur=con.cursor()
...
class Home(tkinter.Tk):
    ...
    def vw(self):
        class Vw(tkinter.Frame):
            ...
            def CreateUI(self):
                tv=Treeview(self)
                tv['column']=('number','org','nature','opennings','place','salary','rattings','employee')
                ...
                tv.grid(sticky=(tkinter.N,tkinter.S,tkinter.E,tkinter.W))
                self.treeview=tv
    ...
    def ins(self):
        ...
        con.autocommit(True)
        if ack !=0:
            messagebox.showinfo('info'," company inserted")
            self.clear()
        else:
            messagebox.showinfo('info',"company can t inserted")
